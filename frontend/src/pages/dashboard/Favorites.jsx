          ...
          {favorites.map((fav) => (
            <Card key={fav.id} className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  {fav.provider && (
                    <Link to={`/providers/${fav.provider}`} className="font-medium text-gray-900 hover:text-primary-600">
                      {fav.provider_name || fav.provider_username}
                    </Link>
                  )}
                  {fav.ad && (
                    <Link to={`/services/${fav.ad}`} className="font-medium text-gray-900 hover:text-primary-600">
                      {fav.ad_title}
                    </Link>
                  )}
                  <p className="text-xs text-gray-400 mt-1">{new Date(fav.created_at).toLocaleDateString('fr-FR')}</p>
                </div>
                <button onClick={() => removeFav(fav.id)} className="p-2 text-gray-400 hover:text-red-500 transition">
