          ...
          {conversations.map((conv) => {
            ...
            return (
              <Link key={conv.id} to={`/dashboard/messages/${conv.id}`}>
                <Card className={`p-4 hover:shadow-md transition ${conv.unread_count > 0 ? 'border-primary-300 bg-primary-50/30' : ''}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        {conv.ad_title && <p className="text-xs text-primary-600 truncate">Re: {conv.ad_title}</p>}
