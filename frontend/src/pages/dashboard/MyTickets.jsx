  ...
  return (
    <DashboardLayout>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 px-4 py-2.5 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700">
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-white rounded-xl shadow-sm border p-6 mb-6">
          ...
          <div className="flex gap-2">
            ...
            <button type="submit" className="px-6 py-2 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700">Créer</button>
          </div>
        </form>
      )}
            ...
            <div key={t.id} className="bg-white rounded-xl shadow-sm border overflow-hidden">
              ...
              {expanded === t.id && detail && (
                <div className="border-t px-5 py-4">
                  <p className="text-sm text-gray-700 mb-4">{detail.description}</p>

                  {detail.responses?.length > 0 && (
                    <div className="space-y-3 mb-4">
                      <p className="text-xs font-medium text-gray-500 uppercase">Conversation</p>
                      {detail.responses.map((r) => (
                        <div key={r.id} className={`p-3 rounded-lg text-sm ${r.is_internal ? 'bg-yellow-50 border-yellow-200 border' : 'bg-gray-50'}`}>
                          <div className="flex justify-between mb-1">
                            <span className="font-medium text-gray-700">{r.author_name || 'Support'}</span>
